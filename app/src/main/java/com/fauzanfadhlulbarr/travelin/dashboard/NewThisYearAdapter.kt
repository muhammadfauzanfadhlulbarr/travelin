...
class NewThisYearAdapter(private var data: List<Palace>,
                         private val listener: (Palace) -> Unit)
    : RecyclerView.Adapter<NewThisYearAdapter.ViewHolder>() {

    lateinit var contextAdapter: android.content.Context

    override fun onCreateViewHolder(parent: ViewGroup,
                                    viewType: Int)
    : NewThisYearAdapter.ViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        contextAdapter = parent.context
        val inflatedView: View = layoutInflater.inflate(R.layout.row_new_this_years, parent, false)

        return ViewHolder(inflatedView)
    }

    override fun onBindViewHolder(holder: NewThisYearAdapter.ViewHolder, position: Int) {
